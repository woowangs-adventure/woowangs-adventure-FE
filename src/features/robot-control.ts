...
export function velocityFromKeys(keys: Set<string>, speedPercent: number) {
  ...
  const linear =
    Number(pressed(keys, "w", "arrowup")) -
    Number(pressed(keys, "s", "arrowdown"))
  const angular =
    Number(pressed(keys, "a", "arrowleft")) -
    Number(pressed(keys, "d", "arrowright"))
...
export function useRobotControl({
  ...
  const stopSender = useRef<(() => void) | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/api/v1/control/capabilities"))
      .then(async (response) => {
        if (!response.ok)
          throw new Error("조작 기능 정보를 확인할 수 없습니다.")
      ...
      .catch((reason) => {
        if (active)
          setError(
            reason instanceof Error ? reason.message : "조작 기능 확인 실패",
          )
      })
    return () => {
      active = false
  ...
  useEffect(() => {
    ...
    socket.onmessage = (message) => {
      ...
      if (event.type === "control.acquired") {
        ...
        acquired = true
        stopSender.current = () => send("control.stop")
        setState("ready")
      } else if (event.type === "control.sent") {
        const commandId = event.data?.command_id
        if (typeof commandId === "string" && !earlyAcks.delete(commandId))
          pendingCommands.set(commandId, performance.now())
        ...
        if (event.data?.applied === false) {
          ...
          setState("error")
          setError(
            String(event.data?.error ?? "로봇이 명령을 실행하지 못했습니다."),
          )
        }
        const commandId = event.data?.command_id
        ...
        setState("error")
        setError(
          String(event.data?.message ?? "로봇 조작 명령이 거부되었습니다."),
        )
      }
    }
    socket.onerror = () => {
  ...
  return {
    ...
    latencyMs,
    stop,
    setHeadlight: (on: boolean) => lightSender.current?.(on),
  }
}
